...
| **H** | **SPLIT THREE WAYS at the map.** ~~H1: **#08 Shopping** + **#07 Settings row** + R104's scan~~ — **SHIPPED v4.04**. **H2: #37 Origins + the map + the Passport hub-row removal** — gated on R106's outline artifact (ledger §4). **H3: #09 landing** — last, because R19's zero-tea Origins state cannot be written before Origins exists | none — `user_settings.settings` is a JSON blob |
