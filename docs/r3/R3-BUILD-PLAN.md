...
| **H** | **SPLIT THREE WAYS at the map.** ~~H1: **#08 Shopping** + **#07 Settings row** + R104's scan~~ — **SHIPPED v4.04**. ~~**H2: #37 Origins + the map + the Passport hub-row removal**~~ — **SHIPPED v4.07** at direction 2 (R106 built; R45/R66 discharged). **H3: #09 landing** — last, because R19's zero-tea Origins state cannot be written before Origins exists | none — `user_settings.settings` is a JSON blob |
