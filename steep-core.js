...
const WHATS_NEW = "When the reference knows exactly where one of your teas grew, the tea form now offers that place instead of just the country — and you can take it or leave it.";
...
/* The UNOPENED predicate — the opposite end of the same v3.40 evidence axis as isTeaFinished
   (fully consumed). It intentionally does NOT scan sessions: its caller (the rediscovery gate) only
   reaches it for never-sessioned teas, so stock is the whole question there.

   v3.98 (spec §7.1) — SAME LADDER SHAPE AS THE FRESHNESS CLOCK, and for the same reason. This used
   to be the authority and read stock evidence alone; `opened_date` is a SECOND definition of the
   same fact, and the two disagree visibly: seal broken, date logged, grams not yet drawn down, and
   Home would say "still unopened" while Tea detail says "opened six weeks ago". So the measured
   answer wins outright and stock inference becomes the FALLBACK rung, exactly as harvest is the
   clock's fallback. Still one authoritative definition — the rungs are ordered inside it, never a
   second inline copy that could drift. Live for 3 teas that read unopened by stock today. */
function isTeaUnopened(tea){
  if(tea && tea.openedDate) return false;                    // measured: the pouch is open, full stop
  const orig=Number(tea&&tea.costOriginalGrams)||0;
  return orig<=0 || (Number(tea.amountGrams)||0) >= orig;    // fallback: stock evidence, as before
}
