...
- Suites: **18 committed, all green** after the second commit in this deploy repairs `status-line` and `tea-types` (below). Local-only `freshness-test.js` and `lifecycle-test.js` stay red for the same stale-expectation reason and are untracked, so a fresh clone sees neither.

### v3.96b — the stale-suite repair (same deploy, second commit)
Also deployed: `steep-tea-types.js` (one `covers` string), `fixtures/status-line-test.js`, `fixtures/tea-types-test.js`. No version or cache bump — same deploy.
