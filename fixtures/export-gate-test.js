...
console.log('\n' + T[1] + ' · not-the-stale-set floors (2026-07-19 authority values)');
ok(S.length  >= 31,  `sessions >= 31  — loaded ${S.length}`);
ok(ST.length >= 103, `steeps   >= 103 — loaded ${ST.length}`);
ok(TE.length >= 22,  `tea rows >= 22  — loaded ${TE.length}`);
ok(V.length  >= 5,   `vessels  >= 5   — loaded ${V.length}`);
ok(W.length  >= 1,   `wishlist >= 1   — loaded ${W.length}`);
ok(US.length >= 1,   `user_settings   — loaded ${US.length}`);
