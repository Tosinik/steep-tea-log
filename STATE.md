...
**NOW (just shipped) — v4.07 R3 slice H2: #37 Origins + the Passport removal** (cache **v117**,
...
- **R105's rule is now in the verifier's standing method**: a check that cannot fail is worse than
  no check. **NEXT: H3** — #09 landing, the last slice; do not move `renderLogin()` out of
  `steep-data.js`.
