...
- **LIVE:** code `192e960` + docs `4a75ac0` pushed; Niklas's re-look (the Time/Temp fields fit the evolution
  card on a phone) + Planning's clone-verify passed → flipped STAGED→LIVE together with v4.49.

**Previously — v4.49 LIVE `41bd57c` — Tea Tasting mode (guided mode c3): the evolution loop + the phantom-steep fix (R191)**
...
- **LIVE:** code `41bd57c` + docs `7e9d02f` pushed. Niklas's phone-look found the evolution steep card's
  Time/Temp grid overflowing the right edge on a phone → fixed in **v4.50**; the combined re-look + Planning's
  clone-verify passed → **v4.49 + v4.50 flipped STAGED→LIVE together.** The guided tasting mode (D4) is COMPLETE
  and LIVE.
...
- **LIVE:** code `04e7536` + docs `2562729` pushed; Niklas's re-look (the huigan/hou-yun ⓘ no longer clip
  left on a narrow phone) + Planning's clone-verify passed → flipped STAGED→LIVE together with v4.47.

**Previously — v4.47 LIVE `6916bd9` — Tea Tasting mode (guided mode c2): fleshed axes, glossary, tradition lens (R189)**
...
- **LIVE:** code `6916bd9` + docs `9825430` pushed. Niklas's phone-look found the glossary ⓘ clipping off the
  LEFT edge on a narrow phone → fixed in **v4.48**; the combined re-look + Planning's clone-verify passed →
  **v4.47 + v4.48 flipped STAGED→LIVE together.**
